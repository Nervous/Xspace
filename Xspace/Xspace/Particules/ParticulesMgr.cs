...
namespace Xspace.Particules
{
    public class ParticulesMgr : DrawableGameComponent
    {
        ...
        public ParticulesMgr(ParticulesGame game, ParticulesOptions options)
            : base(game)
        {
            _game = game;
            _options = options;
            ...
            for (int i = 0; i < _options.Max; i++)
                _particulesListe.Add(new Particule(this) { Vitesse = Vector2.One });
        }

        protected override void LoadContent()
        {
            _texture_particule = _game.Content.Load("particle");
        }
